...
def create_or_refresh_monitoring(config, spark, workspace):
    ...
    inf_table = spark.sql(f"SELECT * FROM {config.catalog_name}.{config.schema_name}.`default_ccc-model-serving_payload_payload`")
    ...
    df_exploded = inf_table_parsed.withColumn("record",
                                    ...
                                    .withColumn("prediction", 
                                                F.col("parsed_response.predictions")[F.col("row_index")])
    ...
    test_set_unique = test_set \
        .withColumn("row_number", F.row_number().over(window_update_timestamp)) \
        .filter(F.col("row_number") == 1) \
        .drop("row_number")

    df_final_with_status = df_final \
        .join(test_set_unique.select("ID", "default_payment_next_month"), on="ID", how="left") \
